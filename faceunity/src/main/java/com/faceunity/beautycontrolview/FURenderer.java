...
public class FURenderer implements OnFaceUnityControlListener {
     ...
     * 销毁faceunity相关的资源
     */
    public void onSurfaceDestroyed(boolean isDeleteThread) {
...
//          faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "blur_blend_ratio", 1);
    ...
    private FullFrameRect mFullScreenFUDisplayOES,
            mFullScreenFUDisplay2D;
    ...
    public void loadItems() {
        ...
        mFullScreenFUDisplayOES = new FullFrameRect(new Texture2dProgram(
                Texture2dProgram.ProgramType.TEXTURE_EXT));

        mFullScreenFUDisplay2D = new FullFrameRect(new Texture2dProgram(
    ...
    public void destroyItems() {
        ...
        onSurfaceDestroyed(true);

        deleteFBO();
    }

    public void destroyItems2() {
        isActive = false;

        onSurfaceDestroyed(false);
    ...
    public int onDrawFrameFBODoubleInput(byte[] img, int tex, int texWidth,
        ...
        mFullScreenFUDisplayOES.drawFrame(tex, matrix);

        GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, fbo[0]);
        GLES20.glViewport(originalViewPort[0], originalViewPort[1], originalViewPort[2], originalViewPort[3]);

        int fuTex = onDrawFrameFBODoubleInput(img,
                fboTex[0], texWidth, texHeight);
        return fuTex;
    }
